...
public class FederatedStochasticProcessDiscovery {
        ...
        for(int i=0;i<numberOfNodes;i++)
        	if (OPTFlag) {	
        		...
        		if (optName.compareTo("PSO") == 0)
        			...
        			optimiser.add(new Genetic(0, iteration, fileDirectory, population, 0.8, 0.0,lower,upper,Frontier_List_Size,time,seconds,globalActions,eventLog,actions.size(),bkgt,optModel,sizeLimit,cof));
        			...
        			optimiser.add(new DifferentialEvolution(0, iteration, fileDirectory, population, 0.5, 0.9,lower,upper,Frontier_List_Size,time,seconds,globalActions,eventLog,actions.size(),bkgt,optModel,sizeLimit,cof));
        		else if (optName.compareTo("GSA") == 0)
        			optimiser.add(new GravitationalSearch(0, iteration, fileDirectory, population, 100,lower,upper,Frontier_List_Size,time,seconds,globalActions,eventLog,actions.size(),bkgt,optModel,sizeLimit,cof));
    ...
    public static void executeFederatedStochasticProcessDiscovery(String []args)
    {
    	...
    	double cof = Integer.parseInt(parms.get("cof"));	

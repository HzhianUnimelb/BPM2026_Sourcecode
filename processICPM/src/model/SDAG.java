...
//	                          .put(alias, x);
	...
	public static FPTA DFFAtoSDAG(FPTA dffa)
	{
		dffa.calculateTransitionPercentage(dffa);
		for(String state:dffa.states)
    		for(String alpha:dffa.alphabet)
    		{
    			if(dffa.getTransitionFunction().get(state+alpha)!=null)
    			{
    				String next = dffa.getTransitionFunction().get(state+alpha);
    				try {
    					dffa.transitionPercentage.get(state).get(alpha).get(next);
    				}catch(Exception e)
    				{
    					dffa.transitionFunction.remove(state+alpha);
    				}
...
//                          .put(alias, x);
   ...
   /*     for(String symbol:list.keySet())
        ...
        //						System.out.println(target.substring(0,targetIndex+1)+"---"+target.substring(targetIndex+2));
        						...
        						dffa.transitionFrequencies.get(target.substring(0,targetIndex)).get(sym).get(target.substring(targetIndex+1));

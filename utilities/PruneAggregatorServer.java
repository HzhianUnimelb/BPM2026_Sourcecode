...
public class PruneAggregatorServer {
	...
	public static void executeFederatedStochasticProcessDiscovery(String []args)
    {
		...
		{		
			...
			PruneAggregatorServer aggregatorServer = new PruneAggregatorServer(numberOfNodes,maxItr,popSize,alg,fileDirectory,true,true,lower_f,upper_f,pareto_size,"d", LocalDateTime.now(),timeLimit,bkgt,parms.get("Optimal Model"),sizeLimit);		
		}
    }	
	/*-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+*/
	public PruneAggregatorServer(int numberOfNodes,int iteration,int population,String optName,String fileDirectory,boolean ParetoFront,boolean OPTFlag,double lower,double upper,int Frontier_List_Size,String symbol,LocalDateTime time,int seconds,BackGroundType bkgt,String optModel,int sizeLimit) {
	     ...
	     double avgEr=0;
	     int activeClients=0;
	     System.out.println("after---> prunn");
			System.out.println(" after prune "+ sdf.format(new Date()));
	     ...
	     System.out.println("Size--->"+clients.get(0).getGlobalModel().getSize());
	     System.out.println("avgER("+avgEr/activeClients+")"+" "+activeClients);
	...
	public FPTA runAlgorithm() {
		...
		aggFPTA.show(aggFPTA, "before prunn");
		System.out.println(" before prune"+ sdf.format(new Date()));
		System.out.println("Size--->"+clients.get(0).getGlobalModel().getSize());
	    System.out.println("avgER("+avgEr/(index-1)+")");
	    Set<String> prunedList = SubgraphSolver.solvePruneOptimization(aggFPTA,clientErEffects,clientEr,0.1);
	 	for(String pnode:prunedList)
	 		aggFPTA.deleteState(aggFPTA, pnode);
	 	aggFPTA.show(aggFPTA, "after prunn");
		 return aggFPTA;
